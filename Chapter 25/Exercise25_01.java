 ...
 * Date: 03/31/2021
 * 
 * This program was edited to add in two new BST methods, height and breadthFirstTraversal which 
 * find the height of a given tree and traverse a tree breadth first respectively. 
 */
		...
		public int height(TreeNode node) {
			...
			if (node == null) return height;
				...
				if (height(node.right) > height(node.left)) {
					height = height(node.right) + 1;
					
				}
				else if (height(node.right) < height(node.left)) {
					height = height(node.left) + 1;
				}
				else {
					height = height(node.right) + 1;
				}
