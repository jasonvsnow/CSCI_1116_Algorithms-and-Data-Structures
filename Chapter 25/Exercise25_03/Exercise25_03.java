...
//    tree.inorder();
  ...
  public static class BST<E extends Comparable<E>> extends AbstractTree<E> {
    ...
    public void nonRecursiveInorder() {
    	Stack<TreeNode<E>> hold = new Stack<TreeNode<E>>();
    	...
    	while (!hold.empty()) {
    		...
    		if (current.left != null && !compare.contains(current.left)) {
    			hold.push(current.left);
    		}
    		else {
    			compare.add(current);
    			System.out.print(hold.pop().element + " ");
    			if (current.right != null && !compare.contains(current.right)) {
    				hold.push(current.right);
    			}
